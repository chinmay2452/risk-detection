...
function getRecommendations(risk) {
  ...
  if (fullText.includes("sql") || fullText.includes("injection")) {
    ...
    recommendations.push(
      "Consider using an Object-Relational Mapper (ORM) to abstract database queries.",
    );
  }
  // Sensitive Data Exposure / Missing Encryption
  else if (
    fullText.includes("sensitive data exposure") ||
    fullText.includes("missing encryption") ||
    fullText.includes("unencrypted") ||
    fullText.includes("plaintext")
  ) {
    recommendations.push(
      "Encrypt sensitive data at rest using strong encryption algorithms (e.g., AES-256).",
    );
    recommendations.push(
      "Enforce TLS 1.2 or higher for all communication involving sensitive data.",
    );
    recommendations.push(
      "Implement robust access controls and least privilege principles for data access.",
    );
  }
  // Missing API Layer / Incomplete Architecture
  else if (
    fullText.includes("missing api") ||
    fullText.includes("missing api layer") ||
    fullText.includes("incomplete architecture")
  ) {
    recommendations.push(
      "Define explicit REST or GraphQL APIs for all interactions between major components.",
    );
    recommendations.push(
      "Introduce an API Gateway to centralize authentication, rate limiting, and routing.",
    );
    recommendations.push("Enforce strict authentication and authorization at the API layer.");
  }
  // Insecure Communication
  else if (
    fullText.includes("insecure communication") ||
    fullText.includes("http") ||
    fullText.includes("untrusted")
  ) {
  ...
  // Cross-Site Scripting (XSS)
    ...
    recommendations.push(
      "Contextually encode all user-supplied output before rendering it in the browser.",
    );
    recommendations.push(
      "Implement a strong Content Security Policy (CSP) to restrict active content sources.",
    );
    recommendations.push("Use modern web frameworks that automatically escape XSS by design.");
  }
  // Authentication / Authorization Bypass
  else if (
    fullText.includes("auth bypass") ||
    fullText.includes("authentication") ||
    fullText.includes("authorization") ||
    fullText.includes("hardcoded")
  ) {
    recommendations.push(
      "Implement a centralized, robust identity and access management (IAM) solution.",
    );
    recommendations.push(
      "Ensure all endpoints perform strict authorization checks on every request.",
    );
    recommendations.push(
      "Remove any hardcoded credentials and use a secure secrets management vault.",
    );
  }
  // Default / Fallback Generic
  else {
    recommendations.push(
      "Conduct a targeted security review and threat model of the affected component.",
    );
    recommendations.push(
      "Ensure the component follows secure by design principles and least privilege.",
    );
    recommendations.push(
      "Implement extensive logging and monitoring to detect anomalous behavior.",
    );
  }
  ...
  if (risk.affected_components && risk.affected_components.length > 0) {
    ...
    recommendations.push(
      `Apply targeted patching and security hardening specifically to: ${componentsList}.`,
    );
  }
...
function generateMitigations(risks) {
  ...
  return risks.map((risk) => {
    return {
      ...
      // Preserve AI/ML score for frontend "AI Score" column compatibility
      severity_score: typeof risk.severity_score === "number" ? risk.severity_score : null,
      score:
